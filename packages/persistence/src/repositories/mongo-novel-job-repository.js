...
exports.MongoNovelJobRepository = void 0;
const novel_job_1 = require("../models/novel-job");
class MongoNovelJobRepository {
    model;
    session;
    constructor(model = novel_job_1.NovelJobModel, session) {
        this.model = model;
    ...
    async initializeJob(input) {
        const doc = await this.model
            .findOneAndUpdate({ jobId: input.jobId }, {
            $set: {
                queue: input.queue,
                status: 'running',
    ...
    async saveGenerationResult(result) {
        const payload = extractPayloadFromContext(result);
        const doc = await this.model
            .findOneAndUpdate({ jobId: result.jobId }, {
            $set: {
                queue: result.queue,
                status: 'completed',
                payload,
...
exports.MongoNovelJobRepository = MongoNovelJobRepository;
function extractPayloadFromContext(result) {
...
function toAggregate(doc) {
    ...
    const context = JSON.parse(JSON.stringify(doc.context ?? {}));
...
function mapChapterAttempts(attempts) {
...
function buildProgress(outline, chapters, failures) {
